...
try:
  ...
  st.error(f"[ERROR] Failed to load menu: {str(e)}")
  menu_items = []
# [END load_menu]
...
if "messages" not in st.session_state:
  st.session_state.messages = [
    {
      "role": "assistant",
      "content": "Welcome to the Coffee Shop! What can I get for you today?"
    }
  ]
...
# User input
if prompt := st.chat_input("Ask for recommendations (e.g. 'What dairy-free pastries do you have?' or 'I want something strong and hot')"):
  ...
  with st.chat_message("assistant"):
    try:
      ...
      async def fetch_response():
        return await st.session_state.runner.run_debug( prompt, session_id=st.session_state.session_id )
      
      res_events = asyncio.run(fetch_response())
      
      response_text = "".join([
        part.text for event in res_events
      ...
      st.error(f"[ERROR]: Apologies, I ran into an error {str(e)}")
